    ...
    const initRoom = async () => {
      try {
        ...
        socket.on('user-joined', (payload) => {
          ...
          // FIX BUG: If peer already exists, this is a renegotiation signal (like turning on camera).
          // We must forward it to the existing peer instead of creating a new one!
          const existingPeerObj = peersRef.current.find(p => p.peerID === callerID);
          if (existingPeerObj && !existingPeerObj.peer.destroyed) {
            try {
  ...
  return (
    <div className="h-[100dvh] bg-gray-950 flex flex-col lg:flex-row overflow-hidden text-gray-100 font-sans">
      ...
      <div className="flex-1 flex flex-col relative min-h-[50dvh] lg:min-h-0">
        ...
        {/* Controls */}
        <div className="h-20 bg-gray-900 border-t border-gray-800 flex items-center justify-center space-x-4 sm:space-x-6 z-10 px-2">
          {(isAdmin || streamRef.current) && (
            <>
              ...
              <button onClick={toggleScreenShare} className={`flex h-12 w-12 rounded-full items-center justify-center transition ${isScreenSharing ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-800 hover:bg-gray-700'}`} title="Share Screen">
                <Monitor className="h-5 w-5" />
              </button>
            </>
          )}
          <button onClick={() => setShowMobileSidebar(true)} className="lg:hidden h-12 w-12 rounded-full bg-gray-800 flex items-center justify-center hover:bg-gray-700 transition text-green-400">
      ...
      <div className={`fixed inset-y-0 right-0 z-40 w-full sm:w-96 bg-gray-900 border-l border-gray-800 flex flex-col transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${showMobileSidebar ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Mobile Header with Close Button */}
        <div className="lg:hidden p-4 border-b border-gray-800 flex items-center justify-between bg-gray-950">
          <h2 className="font-bold text-white">Class Options</h2>
          <button onClick={() => setShowMobileSidebar(false)} className="text-gray-400 hover:text-white text-2xl leading-none">&times;</button>
        </div>
        {isAdmin && (() => {
...
const VideoPlayer = ({ peer, user, isLocalUser, isGranted, chatProps }) => {
  ...
  return (
    <div 
      ref={containerRef}
      className={`group relative rounded-2xl overflow-hidden bg-gray-900 border border-gray-800 flex items-center justify-center transition-all duration-300 ${isBig ? 'w-full lg:w-[calc(50%-1rem)] aspect-square sm:aspect-video shadow-lg' : 'w-32 h-24 border-dashed opacity-70'} ${user.role === 'admin' ? 'hover:ring-2 hover:ring-blue-500' : ''}`}
    >
      <video playsInline autoPlay ref={ref} className={`h-full transition-all duration-500 ${(showCameraOff || (!effectiveHasStream && user.role !== 'admin')) ? 'hidden' : ''} ${isFullscreen ? 'object-contain bg-black' : 'object-cover'} ${isFullscreen && showChat ? 'w-[calc(100%-20rem)] mr-auto' : 'w-full'}`} />
      ...
      {user.role === 'admin' && (
        <div className="absolute top-4 right-4 z-20 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex space-x-2">
          {isFullscreen && (
